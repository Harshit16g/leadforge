...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      >
        <Command shouldFilter={false} className="[&_[data-slot=command-input-wrapper]]:border-b [&_[data-slot=command-input-wrapper]]:px-5 [&_[data-slot=command-input-wrapper]]:py-3 [&_[data-slot=command-input-wrapper]]:h-auto [&_[data-slot=command-input]]:text-base">
          {children}
        </Command>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div data-slot="command-input-wrapper" className="p-1 pb-0">
      <InputGroup className="h-9 bg-input/50">
        ...
        />
        <InputGroupAddon>
          <span className="icon-[solar--magnifer-linear] size-4 shrink-0 opacity-50" />
        </InputGroupAddon>
      </InputGroup>
    </div>
  )
...
function CommandItem({
  ...
  return (
    ...
    >
      {children}
      <span className="icon-[solar--check-read-linear] ml-auto size-4 opacity-0 group-has-data-[slot=command-shortcut]/command-item:hidden group-data-[checked=true]/command-item:opacity-100" />
    </CommandPrimitive.Item>
  )
}
