...
export interface TimeSpan {
  ...
  date?: string       // "YYYY-MM-DD" — populated when loaded from staff_availability
  block_type?: string // "available" | "break" | "lunch" | etc.
...
interface AvailabilityProps {
  ...
  getSlotClassName?: (span: TimeSpan) => string // per-span override for slotClassName
  onSpanClick?: (span: TimeSpan) => void
  className?: string
}
...
const mergeAdjacentSpans = (spans: TimeSpan[]): TimeSpan[] => {
  ...
  spans.forEach(span => {
    const day = span.week_day ?? 0
    const daySpans = byDay.get(day) || []
    daySpans.push(span)
    byDay.set(day, daySpans)
  })
  ...
  byDay.forEach(daySpans => {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      // Check if current and next are adjacent (touching) AND same block type
      if (current.end_time === next.start_time && current.block_type === next.block_type) {
    ...
    >
      ...
      >
        <div
          ...
          className={cn(
            "flex h-full min-h-[480px] w-full flex-col overflow-hidden rounded-xl border bg-background select-none touch-none",
            className,
          )}
        >
          {/* Header */}
...
interface DayColumnProps {
  ...
  getSlotClassName?: (span: TimeSpan) => string
  onSpanClick?: (span: TimeSpan) => void
}
...
function DraggableTimeSpan({
  ...
  const content = (
    <>
      ...
      {canResize && (
        <div
          className="absolute top-0 left-0 right-0 h-5 -mt-2.5 cursor-row-resize z-20"
      ...
      {canResize && (
        <div
          className="absolute bottom-0 left-0 right-0 h-5 -mb-2.5 cursor-row-resize z-20"
  ...
  return (
    <>
      <div
        ...
        className={cn(
          "absolute left-1 right-1 rounded-lg border-2 px-2 py-1.5 shadow-md text-xs group overflow-hidden touch-none z-10",
          getSlotClassName ? getSlotClassName(span) : slotClassName,
      ...
      {isDragging && context && (
        <context.dragPreviewTunnel.In>
          <div
            className={cn(
              "absolute left-0 right-0 rounded-lg border-2 px-2 py-1.5 shadow-xl text-xs overflow-hidden h-full w-full",
              context.isDropValid ? "border-primary/60 bg-primary/15" : "border-destructive/60 bg-destructive/20",
            )}
          >
...
function TimeSpanCard({
  ...
  const isShort = calculatedDuration < 0.65 // Less than 40 minutes

  if (isShort) {
    return (
      <div className="h-full flex items-center justify-between text-foreground timespan-inner-area pointer-events-none px-1">
        <div className="flex items-center gap-1 min-w-0">
          <span className={cn(
            "size-3 shrink-0 opacity-80",
            span.block_type === "break" ? "icon-[solar--cup-hot-bold-duotone]" :
              span.block_type === "lunch" ? "icon-[solar--map-bold-duotone]" :
                "icon-[solar--clock-circle-bold-duotone]"
          )} />
          <p className="text-[9px] font-black tracking-tight truncate leading-none">
            {formatDisplayTime(span.start_time, useAmPm)} – {formatDisplayTime(span.end_time, useAmPm)}
          </p>
        </div>
        {onDelete && (
          <Button
            variant="ghost"
            size="icon"
            className="h-4 w-4 hover:bg-foreground/5 dark:hover:bg-foreground/10 absolute right-0.5 top-1/2 -translate-y-1/2 z-20 pointer-events-auto opacity-0 group-hover:opacity-100 transition-opacity"
            onPointerDown={e => {
              e.stopPropagation()
            }}
            onClick={e => {
  ...
  return (
    <div className="h-full flex flex-col relative items-between text-foreground timespan-inner-area pointer-events-none">
      <div className="flex flex-col gap-0.5 text-inherit">
        ...
        {span.block_type && span.block_type !== "available" && (
          <p className="text-[8px] font-black uppercase tracking-wider opacity-70">{span.block_type}</p>
        )}
      ...
      {onDelete && (
        <Button
          variant="ghost"
          size="icon"
          className="h-5 w-5 hover:bg-foreground/5 dark:hover:bg-foreground/10 -mt-1 -mr-1 absolute top-0 right-0 z-20 pointer-events-auto opacity-0 group-hover:opacity-100 transition-opacity"
          onPointerDown={e => {
